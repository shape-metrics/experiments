...
template <typename DrawingT> class Experiments {
  ...
  protected:
    ...
    compute_drawing(const domus::graph::Graph& graph, std::string_view graph_name) = 0;
    ...
    int process_graph(const domus::graph::Graph& graph, std::string_view graph_filename) {
  ...
  public:
    ...
    void run_experiments() {
        domus::utilities::collect_txt_files(graphs_folder_path)
            .and_then(
                        ...
                        threads.emplace_back([&]() {
                            while (true) {
                                ...
                                if (graphs_to_skip.contains(graph_filename))
                                    continue;
                                auto graph =
                                    domus::graph::loader::load_graph_from_txt_file(entry_path);
                                if (!graph) {
                                    std::print(
                                ...
                                if (!domus::graph::algorithms::is_graph_connected(*graph)) {
