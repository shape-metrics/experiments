...
class ShapeMetricsExperiments : public Experiments<ShapeMetricsDrawing> {
  ...
  public:
    ...
    std::expected<void, std::string>
    save_svg(const ShapeMetricsDrawing& drawing, std::string graph_name) override;
    std::expected<void, std::string>
    save_drawing(const ShapeMetricsDrawing& drawing, std::string graph_name) override;
